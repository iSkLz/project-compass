import { BrowserWindow, ipcMain, MessageChannelMain, Size } from "electron";
...
import { utils, paths, os } from "@libs/helpers";

import { WindowState, WindowOptions, ShowMode, NodeState, MenuState } from "./types.js";
import { preloadFile, preloadScript } from "./preload.js";
...
export const defaultWinOptions: WindowOptions = {
    defaultTitle: "",
...
export default class UIWindow extends EventEmitter {
    ...
    constructor(size: Size,
        ...
        let ID: string;
        while (UIWindow.Instances.has(ID = utils.getRandomID())) {}
        UIWindow.Instances.set(this.ID = ID, this);

        // Assign default options
        let options = utils.assignDefaults(winOptions, defaultWinOptions);

        let webOptions: any;
        for (const key in options) {
            if (key !== "icon" && key !== "menuBar" && key !== "parent")
                webOptions[key] = options[key as keyof WindowOptions];
        }
