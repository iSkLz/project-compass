...
import compass from "@libs/webhelper";
...
export function importLib(lib: string, type: LibType = LibType.web, importType: ImportType = ImportType.nodeJS, defaultExport: boolean = true) {
    ...
    if (compass.options.nodeState != 0) return null;
    
    let libPath: string = lib;
    switch (type) {
        case LibType.web:
            libPath = path.join(compass.rootPath, "web/libs", lib);
            break;
        case LibType.helper:
            libPath = path.join(compass.rootPath, "helpers", lib);
            break;
        case LibType.compass:
            libPath = path.join(compass.rootPath, lib);
            break;
    }
...
export function autoImport() {
    if (compass.options.nodeState) return;
